...
import in.co.school.mgt.exception.RecordNotFoundException;
...
public class UserModel {
	...
	public long nextSchoolCode() throws DatabaseException {
		log.debug("Model nextPK Started");
		Connection conn = null;
		long pk = 0;
		...
		try {
			conn = JDBCDataSource.getConnection();
			PreparedStatement pstmt = conn.prepareStatement("SELECT MAX(schoolId) FROM S_USER");
			ResultSet rs = pstmt.executeQuery();
			while (rs.next()) {
				pk = rs.getInt(1);
			}
	...
	public long add(UserBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			pstmt.setString(14, bean.getProfilePic());
			pstmt.setLong(15,schoolCode);
			pstmt.executeUpdate();
	...
	public UserBean findByLogin(String login) throws ApplicationException {
		...
		try {
			...
			while (rs.next()) {
				...
				bean.setProfilePic(rs.getString(14));
				bean.setSchoolId(rs.getLong(15));
	...
	public void update(UserBean bean) throws ApplicationException, DuplicateRecordException {
		...
		try {
			...
			PreparedStatement pstmt = conn.prepareStatement(
					...
					+ "GENDER=?,"
					+ "CREATEDBY=?,MODIFIEDBY=?,CREATEDDATETIME=?,MODIFIEDDATETIME=?,profilePic=?,schoolId=? WHERE ID=?");
			...
			pstmt.setString(13, bean.getProfilePic());
			pstmt.setLong(14,bean.getSchoolId());
	...
	public List search(UserBean bean, int pageNo, int pageSize) throws ApplicationException {
		...
		if (bean != null) {
			...
			if (bean.getSchoolId() > 0) {
				sql.append(" AND SchoolID = " + bean.getSchoolId());
			}
